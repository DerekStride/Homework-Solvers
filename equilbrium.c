...
void coordinates_to_vector(double result[])
{
	double x[2], y[2], z[2];
	...
	for (int g = 0; g < 2; g++){
			...
			get_three_dimensional_coordinate(X, Y, Z, g + 1);
	}
...
/* Calculates and returns the components of the force along a line. */

void force_along_line(double result[])
{
	double u[3], force, mag, total_x, total_y, total_z;
	
	puts("Enter the magnitude of the Force:");
	scanf("%lf", &force);
	
	coordinates_to_vector(u);
	
	mag = magnitude_mag(u[0], u[1], u[2]);
	
	/* Changes the values of the total_? variables to unit vector components */
	
	total_x = u[0] / mag;
	total_y = u[1] / mag;
	total_z = u[2] / mag;
	
	result[0] = force * total_x;
	result[1] = force * total_y;
	result[2] = force * total_z;
	
	
}
/* Calculates and outputs the force along a line. */

void vector_force_along_line(void)
{
...
void torque_3D(double torque[])
{
	...
	puts("r x F");
	puts("Enter the coordinates for the vector r");
	coordinates_to_vector(r);
	
	/* Sets the value in f to the i, j, and k components of the force */
	do {
			condition = 0;
			puts("1 - Enter the force");
			puts("2 - Enter calculate the force");
			scanf("%d", &input);
			switch (input){
				case 1:
					get_three_dimensional_vector(I, J, K, 1);
					break;
				case 2:
					force_along_line(f);
					break;
...
double scalar_triple_product(void)
{
	...
	/* 
	 * Sets the second index of i, j, and k to the components of torque.
	 */
...
{
	int input, condition;
	double result, torque[3];
	do {
		puts("What would you like to do?");
		puts("1 - Force projected along a vector");
		puts("2 - Forces at Equilibrium (Only Basics, yet)");
		puts("3 - 2D Torque");
		puts("4 - 3D Torque");
		puts("5 - Torque projected along a vector");
		scanf("%d", &input);
		...
		case 4:
				torque_3D(torque);
				output_result_vector(torque[0], torque[1], torque[2]);
				break;
		case 5:
